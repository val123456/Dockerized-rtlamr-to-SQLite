...
for meter in meter_ids:
    ...
    if meter in electric_meter_ids:
        ...
        con.execute('CREATE INDEX IF NOT EXISTS index_{}  ON Water_Meter_{}(date)'.format(str(meter), str(meter)))
        con.execute('CREATE INDEX IF NOT EXISTS index_use_{}  ON Water_DailyUse_{}(date)'.format(str(meter), str(meter)))
            ...
            try:
                ...
                if meter in electric_meter_ids:
                    if last[meter] != round(raw['Message']['Consumption'] / electric_cnv, rnd):
                        ...
                        record['kWattHours'] = round(raw['Message']['Consumption'] / electric_cnv, rnd)
                        ...
                        con.execute(''
                            ...
                            record['Meter'], record['DateTime'].strftime(DATE_FORMAT)))
                        ...
                        if use:
                            con.execute(
                                '''INSERT INTO Electric_DailyUse_{}(date, time, kWHs, fulldatetime) VALUES(?, ?, ?, ?)'''.format(
                                    meter),
                                (data[0], data[1], round(data[2] - use[0][2], rnd), data[3]))
                            db.commit()
                        else:
                            con.execute(
                                '''INSERT INTO Electric_DailyUse_{}(date, time, kWHs, fulldatetime) VALUES(?, ?, ?, ?)'''.format(
                    ...
                    if last[meter] != round(raw['Message']['Consumption'] / gas_cnv, rnd):
                        record['Meter'] = raw['Message']['ID']
                        record['DateTime'] = datetime.datetime.strptime(raw['Time'].split('.', 1)[0], DATETIME_FORMAT)
                        record['ccf'] = round(raw['Message']['Consumption'] / gas_cnv, rnd)
                    ...
                    if last[meter] != round(raw['Message']['Consumption'] / water_cnv, rnd):
                        record['Meter'] = raw['Message']['ID']
                        record['DateTime'] = datetime.datetime.strptime(raw['Time'].split('.', 1)[0], DATETIME_FORMAT)
                        record['cubicft'] = round(raw['Message']['Consumption'] / water_cnv, rnd)
                        ...
                        last[meter] = round(raw['Message']['Consumption'] / water_cnv, rnd)
